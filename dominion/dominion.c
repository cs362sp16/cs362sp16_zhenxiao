...
int fcutpurse(struct gameState *state, int handPos){
		int i;
		int k;
		int j;
...
int fsea_hag(struct gameState *state){
		int i;
		int currentPlayer = whoseTurn(state);
		for (i = 0; i < state->numPlayers; i++){
	if (i != currentPlayer){
...
int fsteward(struct gameState *state, int handPos, int choice1, int choice2, int choice3){
	int currentPlayer = whoseTurn(state);
	if (choice1 == 1)
	{
	  //+2 cards
...
int fcouncil_room(struct gameState *state, int handPos){
	int i;
	int currentPlayer = whoseTurn(state);
	for (i = 0; i < 4; i++)
	{
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case steward:
	fsteward(state, handPos, choice1, choice2, choice3);
      /*if (choice1 == 1)
	{
	  //+2 cards
	  drawCard(currentPlayer, state);
